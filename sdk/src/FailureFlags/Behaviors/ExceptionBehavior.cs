...
namespace FailureFlags
{
    ...
    /// <summary>
    /// ExceptionBehavior processes <code>exception</code> properties in experiment effects.
    ///
    ...
    public class ExceptionBehavior : IBehavior
    {
        ...
        public ExceptionBehavior() : this(null)
        {
        }
        ...
        public void ApplyBehavior(Experiment[] experiments)
        {
            ...
            foreach (var e in exceptions)
            {
                ...
                if (e is Dictionary<string, object> map)
                {
                    ...
                    _logger.LogWarning(
                        "exception effect has no \"message\" property, throwing nothing; its properties were: {Properties}",
                        string.Join(", ", map.Keys));
                    continue;
                }

                _logger.LogWarning(
                    "exception effect is a {Type}, which is neither a string nor an object; throwing nothing",
                    e?.GetType().Name ?? "null");
            }
        }
